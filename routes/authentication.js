...
module.exports=(router) =>{
  ...
  router.post('/register-medcin', (req, res) => {
    ...
    if (!req.body.email) {
      ...
      if (!req.body.username) {
        ...
        if (!req.body.nationalite) {
          res.json({ success: false, message: 'You must provide a nationalite' }); // Return error
        } 
        
        else {
          if (!req.body.specialite) {
            res.json({ success: false, message: 'You must provide a specialite' }); // Return error
          } 
          
          else {
            if (!req.body.diplome) {
              res.json({ success: false, message: 'You must provide a diplome' }); // Return error
            } 
        ...
        if (!req.body.password) {
          ...
          let medcin = new medcin({
            email: req.body.email.toLowerCase(),
            username: req.body.username.toLowerCase(),
            specialite: req.body.specialite.toLowerCase(),
            diplome: req.body.diplome.toLowerCase(),
            nationalite: req.body.nationalite.toLowerCase(),
                ...
                if (err.errors) {
                  ...
                  if (err.errors.email) {
                    ...
                    if (err.errors.username) {
                      ...
                      if (err.errors.specialite) {
                        res.json({ success: false, message: err.errors.username.message }); // Return error
                      } else {
                        if (err.errors.diplome) {
                          res.json({ success: false, message: err.errors.username.message }); // Return error
                        } else {
                          if (err.errors.nationalite) {
                            res.json({ success: false, message: err.errors.username.message }); // Return error
                          } else {
          ...
          if (err) {
            ...
            if (!user) {
              ...
              if (!validPassword) {
                ...
                const token = jwt.sign({ _userId: user._id,
get userId() {
                    return this._userId;
                  },
set userId(value) {
